...
use crate::models::health::health::{
    DietConfig, FoodGroup, FoodVariant, HealthFile, MacroMode, NutritionalInfo, DEFAULT_HEALTH_PATH,
};
...
fn get_exercises_and_burn(
                    ...
                    if !exercise_names.is_empty() {
                        ...
                        if let Some(ex_data) = exercises_map.get(selected_exercise_name) {
                            ...
                            if let Some(examples) =
                                ex_data.get("examples").and_then(|ex| ex.as_object())
                            {
                                if let Some((_, ex_details)) = examples.iter().next() {
                                    if let Some(url) =
                                        ex_details.get("fileLocation").and_then(|s| s.as_str())
                                    {
    ...
    let avg_session_burn = if exercise_count > 0.0 {
        total_burn_rate_sum / exercise_count
    } else {
        0.0
    };
...
fn calc_macro_cals(
    ...
    match mode {
        ...
        MacroMode::Grams => (target, target * cals_per_gram),
    }
}
...
fn run_simulation(
    ...
    while current_weight > target_weight && week < 150 {
        ...
        let (p_g, p_c) = calc_macro_cals(
            diet_config.protein.mode,
            if diet_config.protein.mode == MacroMode::Percentage {
                diet_config.protein.percentage
            } else {
                diet_config.protein.target_grams
            },
            daily_intake,
            4.0,
        );
        let (c_g, c_c) = calc_macro_cals(
            diet_config.carbs.mode,
            if diet_config.carbs.mode == MacroMode::Percentage {
                diet_config.carbs.percentage
            } else {
                diet_config.carbs.target_grams
            },
            daily_intake,
            4.0,
        );
        let (f_g, f_c) = calc_macro_cals(
            diet_config.fats.mode,
            if diet_config.fats.mode == MacroMode::Percentage {
                diet_config.fats.percentage
            } else {
                diet_config.fats.target_grams
            },
            daily_intake,
            9.0,
        );
    ...
    rows.push(SimulationRow {
        week,
        date_display: (start_date + Duration::weeks(week as i64))
            .format("%b %d")
            .to_string(),
        weight: current_weight,
...
/// Insert (or extend) a workout in the untyped workouts map. Building the nested
/// {focus:{group:{exercise:{caloriesPerHour,examples}}}} shape on the fly means
/// "add a workout" and "add an exercise" are the same operation.
fn add_or_extend_workout(
    workouts: &mut HashMap<String, serde_json::Value>,
    workout: &str,
    focus: &str,
    exercise: &str,
    calories_per_hour: f64,
) {
    let root = workouts
        .entry(workout.to_string())
        .or_insert_with(|| serde_json::json!({ "focus": {} }));
    if !root.is_object() {
        *root = serde_json::json!({ "focus": {} });
    }
    let root_obj = root.as_object_mut().unwrap();
    let focus_v = root_obj
        .entry("focus")
        .or_insert_with(|| serde_json::json!({}));
    if !focus_v.is_object() {
        *focus_v = serde_json::json!({});
    }
    let focus_obj = focus_v.as_object_mut().unwrap();
    let group = focus_obj
    ...
    group.as_object_mut().unwrap().insert(
        exercise.to_string(),
        serde_json::json!({ "caloriesPerHour": calories_per_hour, "examples": {} }),
    );
}
...
pub fn Health(#[props(default)] overview: bool) -> Element {
    ...
    use_context_provider(|| HealthCtx {
        health_file,
        file_path,
        status,
    });
    ...
    // Library-editor form drafts (committed on the Add button, parsed there).
    let mut np_name = use_signal(String::new);
    let mut np_unit = use_signal(|| "serving".to_string());
    let mut np_cals = use_signal(String::new);
    let mut np_protein = use_signal(String::new);
    let mut np_fat = use_signal(String::new);
    let mut np_carbs = use_signal(String::new);
    let mut nw_name = use_signal(String::new);
    let mut nw_focus = use_signal(String::new);
    let mut nw_exercise = use_signal(String::new);
    let mut nw_cals = use_signal(String::new);
    ...
    let on_load = {
        ...
        move |_| {
            ...
            match load_json::<HealthFile>(&path) {
                Ok(f) => {
                    health_file.set(f);
                    status.set(Some(format!("Loaded health data from {}", path)));
                }
    ...
    // Push the weekly workout schedule into the shared calendar as one
    // weekly-recurring event per training day. Idempotent: clears prior Health
    // events first so re-syncing never duplicates.
    let on_sync_calendar = move |_| {
        scheduler.remove_by_source(EventSource::Health);

        let t = OffsetDateTime::now_utc().date();
        let today = NaiveDate::from_ymd_opt(t.year(), t.month() as u32, t.day() as u32)
            .unwrap_or_else(|| NaiveDate::from_ymd_opt(2024, 1, 1).unwrap());
        let w_today = today.weekday().num_days_from_monday() as i64;

        let file = health_file.read();
        let durs = daily_durations.read();
        let mut count = 0u32;

        for (i, day) in DAYS_OF_WEEK.iter().enumerate() {
            let Some(sched) = file.schedule.get(*day) else {
                continue;
            };
            let roots: Vec<String> = sched
                .overview
                .iter()
                .filter(|s| !s.is_empty())
                .cloned()
            ...
            let hrs = durs.get(*day).cloned().unwrap_or(0.0);
            // Burn rate keyed off the primary (first) workout, matching daily_burn_map.
            let (_, rate) = get_exercises_and_burn(&file.workouts, &roots[0], 0);
            let burn = (rate * hrs) as i64;

            let title = if hrs > 0.0 {
                format!(
                    "{} \u{2014} {:.1} h (~{} kcal)",
                    roots.join(" + "),
                    hrs,
            ...
            // Anchor the recurrence on the next occurrence of this weekday.
            let delta = ((i as i64) - w_today).rem_euclid(7);
        ...
        status.set(Some(format!(
            "Synced {count} workout day(s) to the calendar"
        )));
    };
            ...
            if let Some(day_sched) = file.schedule.get(*day) {
                if let Some(root) = day_sched.overview.first() {
                    if !root.is_empty() {
                        ...
                        let (_, burn_rate_per_hour) = get_exercises_and_burn(
                            &file.workouts,
                            root,
                            current_week_for_projection,
                        );
                        ...
                        div { class: "flex-col w-full max-w-[1200px]",
                            ...
                            div { class: "flex items-center justify-between mb-4",
                                ...
                                div { class: "flex items-center gap-2 ml-4",
                                    ...
                                    button { class: "px-3 py-1 border border-blue-700 text-blue-300 rounded text-sm hover:bg-blue-900/30", onclick: on_sync_calendar, "Sync to Calendar" }
                                }
                            }
                    ...
                    div {
                        ...
                        div { class: "space-y-1 text-xs text-neutral-400",
                            ...
                            div { class: "flex justify-between", span { "Monounsat. Fat" } span { "{avg_mono as i32} g" } }
                                ...
                                AccordionItem { index: 3usize, AccordionTrigger { class:"flex justify-center w-full bg-neutral-900/30 p-2 rounded mb-1 hover:bg-neutral-800/50", span { class: "font-bold", "Library Editor (Add Pantry & Workouts)" } }
                                    AccordionContent {
                                        div { class: "grid grid-cols-1 lg:grid-cols-2 gap-6 p-4",

                                            // ---- ADD PANTRY ITEM ----
                                            div { class: "border border-neutral-700 rounded-lg p-4 bg-neutral-900/30 space-y-3",
                                                h3 { class: "font-bold text-lg text-neutral-200", "Add Pantry Item" }
                                                div { class: "grid grid-cols-2 gap-2",
                                                    input { class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm col-span-2", placeholder: "Name (e.g. Chicken Breast)", value: "{np_name}", oninput: move |e| np_name.set(e.value()) }
                                                    input { class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Unit (lb, egg, cup)", value: "{np_unit}", oninput: move |e| np_unit.set(e.value()) }
                                                    input { type: "number", class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Calories", value: "{np_cals}", oninput: move |e| np_cals.set(e.value()) }
                                                    input { type: "number", class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Protein (g)", value: "{np_protein}", oninput: move |e| np_protein.set(e.value()) }
                                                    input { type: "number", class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Total Fat (g)", value: "{np_fat}", oninput: move |e| np_fat.set(e.value()) }
                                                    input { type: "number", class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Carbs (g)", value: "{np_carbs}", oninput: move |e| np_carbs.set(e.value()) }
                                                }
                                                button {
                                                    class: "w-full px-3 py-1 border border-green-700 text-green-300 rounded text-sm hover:bg-green-900/30",
                                                    onclick: move |_| {
                                                        let name = np_name.read().trim().to_string();
                                                        if name.is_empty() { return; }
                                                        let info = NutritionalInfo {
                                                            calories: np_cals.read().trim().parse().unwrap_or(0.0),
                                                            protein: np_protein.read().trim().parse().unwrap_or(0.0),
                                                            total_fat: np_fat.read().trim().parse().unwrap_or(0.0),
                                                            carbohydrates: np_carbs.read().trim().parse().unwrap_or(0.0),
                                                            ..Default::default()
                                                        };
                                                        let mut variants = HashMap::new();
                                                        variants.insert("serving".to_string(), FoodVariant::Direct(info));
                                                        health_file.write().nutrition_constants.insert(
                                                            name,
                                                            FoodGroup { enabled: true, r#type: "serving".to_string(), unit: np_unit.read().clone(), variants },
                                                        );
                                                        np_name.set(String::new());
                                                        np_cals.set(String::new());
                                                        np_protein.set(String::new());
                                                        np_fat.set(String::new());
                                                        np_carbs.set(String::new());
                                                    },
                                                    "+ Add Pantry Item"
                                                }
                                                div { class: "border-t border-neutral-800 pt-2 space-y-1 max-h-64 overflow-y-auto",
                                                    for (key, group) in health_file.read().nutrition_constants.iter() {
                                                        div { class: "flex items-center justify-between text-sm",
                                                            span { class: "text-neutral-300", "{key} " span { class: "text-neutral-600 text-xs", "({group.unit})" } }
                                                            button { class: "text-red-400 px-2 hover:text-red-300", onclick: { let key = key.clone(); move |_| { health_file.write().nutrition_constants.remove(&key); } }, "\u{2715}" }
                                                        }
                                                    }
                                                }
                                            }
                                            ...
                                            // ---- ADD / EXTEND WORKOUT ----
                                            div { class: "border border-neutral-700 rounded-lg p-4 bg-neutral-900/30 space-y-3",
                                                h3 { class: "font-bold text-lg text-neutral-200", "Add / Extend Workout" }
                                                p { class: "text-xs text-neutral-500", "Type a new or existing workout name to create it, or add another exercise to it." }
                                                div { class: "grid grid-cols-2 gap-2",
                                                    input { class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Workout (e.g. Push)", value: "{nw_name}", oninput: move |e| nw_name.set(e.value()) }
                                                    input { class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Focus (e.g. chest)", value: "{nw_focus}", oninput: move |e| nw_focus.set(e.value()) }
                                                    input { class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Exercise name", value: "{nw_exercise}", oninput: move |e| nw_exercise.set(e.value()) }
                                                    input { type: "number", class: "bg-neutral-800 border border-neutral-700 rounded p-1 text-sm", placeholder: "Cal / hour", value: "{nw_cals}", oninput: move |e| nw_cals.set(e.value()) }
                                                }
                                                button {
                                                    class: "w-full px-3 py-1 border border-blue-700 text-blue-300 rounded text-sm hover:bg-blue-900/30",
                                                    onclick: move |_| {
                                                        let w = nw_name.read().trim().to_string();
                                                        let fo = nw_focus.read().trim().to_string();
                                                        let ex = nw_exercise.read().trim().to_string();
                                                        if w.is_empty() || fo.is_empty() || ex.is_empty() { return; }
                                                        let cals = nw_cals.read().trim().parse().unwrap_or(0.0);
                                                        add_or_extend_workout(&mut health_file.write().workouts, &w, &fo, &ex, cals);
                                                        nw_exercise.set(String::new());
                                                        nw_cals.set(String::new());
                                                    },
                                                    "+ Add / Extend"
                                                }
                                                div { class: "border-t border-neutral-800 pt-2 space-y-1 max-h-64 overflow-y-auto",
                                                    for (wkey, wval) in health_file.read().workouts.iter() {
                                                        {
                                                            let groups = wval.get("focus").and_then(|f| f.as_object()).map(|o| o.len()).unwrap_or(0);
                                                            rsx! {
                                                                div { class: "flex items-center justify-between text-sm",
                                                                    span { class: "text-neutral-300", "{wkey} " span { class: "text-neutral-600 text-xs", "({groups} focus groups)" } }
                                                                    button { class: "text-red-400 px-2 hover:text-red-300", onclick: { let wkey = wkey.clone(); move |_| { health_file.write().workouts.remove(&wkey); } }, "\u{2715}" }
                                                                }
                                                            }
                                                        }
                                                    }
